...
namespace Microsoft.Ajax.Utilities
{
    ...
    internal CrunchEnumerator(SimpleHashtable verboten)
    {
      // empty hashtable
      m_skipNames = new Dictionary<string,string>(verboten.Count);

      // walk through all the items in verboten
      System.Collections.IDictionaryEnumerator enumerator = verboten.GetEnumerator();
      while (enumerator.MoveNext())
      {
        // if the item is a local field, get the crunched name.
